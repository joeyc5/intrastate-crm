...
- **Live pricing (no Calculate button):** the calculator recomputes as you type — discrete controls (selects/radios/toggles) fire instantly, typed fields debounce 300ms. Gotchas baked into `QuoteForm`: (1) a pending debounce timer must read the **latest** payload via a `payloadRef` set every render — a captured closure clobbers a mid-debounce discrete change (wrong NTE at rest); (2) stale-response guard via a request-id ref (drop superseded responses); (3) keep the **last good** result on invalid/mid-edit — never blank the number; (4) `aria-live` on the figure + `aria-busy` on the panel. `page.tsx` prices the default move server-side so the summary is populated on load.
- **UI shape:** two-pane workspace (inputs left, sticky dark `SummaryPanel` right) — an internal salesperson tool, not customer-facing. Inter via `next/font`; semantic color tokens in `@theme`. The customer-facing premium artifact is the (still-pending) **PDF sales quote**.
...
## Status
Spec audited vs MAX4 2026 (`docs/SPEC-AUDIT-2026.md`); mileage basis = GPS. **Built & merged to `main`:** pricing-engine core (`lib/pricing/`), Next.js quote calculator (`app/`), packing pass (Item 340), accessorials pass (Items 140/164/184/148-156 — flights/long-carry, shuttle, extra-stop, bulky; all tariff-capped + non-discountable), branding/logo, **calculator UI redesign (modern two-pane workspace + live pricing, no Calculate button)**. **Decided:** materials tax 9.25% (Santa Clara); packing labor = hourly (per hour per person; pack=origin, unpack=dest); accessorials are non-discountable (discount lever reduces line-haul only); the calculator UI is an internal salesperson tool (premium customer-facing artifact = the PDF quote). **Next:** printed/PDF sales quote → Save & CRM (Supabase) → Google Distance Matrix auto-mileage. Open: BHGS ACV-$20k default chargeability; confirm with SVM that shuttle/extra-stop/bulky should bill at full tariff (non-discountable) — flights/long-carry must be (Item 140 NOTE 3).
